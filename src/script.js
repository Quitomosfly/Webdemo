...
document.addEventListener("DOMContentLoaded", function () {
    async function loadEventDetails() {
        ...
        console.log("Extracted eventId from URL:", eventId); // Debugging step

        if (!eventId || eventId.length !== 24 || !/^[a-f0-9]{24}$/.test(eventId)) {
            console.error("Invalid or missing event ID in URL:", eventId);
            alert("Invalid or missing event ID. Check the URL.");
            return;
        }
